...
--------------------------------------------------------
  ...
  CREATE INDEX "SPC_REPORT_DETAIL_INDX1" ON "SPC_REPORT_DETAIL" ("REPORT_ID", "SEQ_NO");
...
begin
    if inserting and :new.created is null then
        ...
        select nvl(max(rd.seq_no) + 10, 10)
        into :new.seq_no
        from spc_report_detail rd
        where rd.report_id = :new.report_id;
    end if; 
